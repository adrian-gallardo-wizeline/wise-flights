...
function obtenerPrecios(fechaIda, fechaVuelta) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const url = `https://www.kayak.com.mx/flights/${origen}-${destino}/${fechaIdaDesde}/${fechaVueltaHasta}?sort=bestflight_a`;
      await page.goto(url);
      await waitForElement(page, '.best-flights-header');
      const recomendedPriceEl = await waitForElement(page, '.Common-Booking-MultiBookProvider  .price');
      const recomendedPrice = await getInnerText(recomendedPriceEl);
      console.log('%%%%%%%', recomendedPrice);
      const recomendedDurationEl = await waitForElement(page, '.keel-grid .duration .top');
      const recomendedDuration = await getInnerText(recomendedDurationEl);
      console.log('%%%%%%%', recomendedDuration);
      const bestPriceEl = await waitForElement(page, '.Common-Booking-MultiBookProvider  .price');
      const bestPrice = await getInnerText(bestPriceEl);
      const bestDurationEl = await waitForElement(page, '.keel-grid .duration .top');
